...
def match_taxonID(listofdataframe, listofdfname):
    ...
    for i in range(len(listofdataframe)):
        listofdataframe[i]['attribute'] = listofdataframe[i][['Kingdom', 'Phylum', 'Class', 'Order']].astype(
...
rsdf1.to_csv('result/multikeymapping_fOrder.csv', index=False)
rsdf2.to_csv('result/nonkeymapping_fOrder.csv', index=False)
...
mappingdf = pd.read_excel('result/taxonIDmappingdict_fFamily.xlsx')
df = pd.read_csv('./Datasets/processing_4_microbiome/microbiome_combine_790row_480col.csv')

def Directlyreplace_keyid(mapping, df):
    mapping['Order_attr'] = mapping['Family_attr'].apply(lambda x: x.split(',')[:-1]).apply(lambda x: ','.join(x))
    mapping['taxonID'] = mapping['taxonID'].apply(lambda x: x.strip('[]'))
    mapping['newtaxonID'] = mapping['newtaxonID'].apply(lambda x: x.strip('[]'))
    newmapping = mapping.groupby(['Order_attr'], as_index=False)[['FamilytaxonID','newtaxonID', 'taxonID']].agg(lambda x: ','.join(x)).reset_index()
    sequence = [f"{i:03}" for i in range(1, newmapping.shape[0]+1)]
    newmapping['OrdertaxonID'] = list(sequence)[:newmapping.shape[0]]
    newmapping['OrdertaxonID'] = 'Order' + newmapping['OrdertaxonID'].astype(str)
    newmappingdf = newmapping[['OrdertaxonID', 'FamilytaxonID','newtaxonID', 'taxonID', 'Order_attr']]
    newmappingdf = newmappingdf.reset_index(drop=True)
    # print("lenght:", newmappingdf['newtaxonID'].apply(lambda x: x[0].strip('[]')))

    newtaxonIDALLlist = [f"NEWASV{i:03}" for i in range(1, 481)]
    result = {}
    for col in newtaxonIDALLlist:
        df[col] = df[col].astype(float)
        for j in range(len(newmappingdf)):
            newtaxlist = newmappingdf.loc[j, 'newtaxonID'].split(',')
            print("aaaaaa:", newtaxlist)
            if col in newtaxlist:
                if newmappingdf.loc[j, 'OrdertaxonID'] in result:
                    result[newmappingdf.loc[j, 'OrdertaxonID']] += np.array(df[col])
                else:
                    result[newmappingdf.loc[j, 'OrdertaxonID']] = np.array(df[col])
...
mappingdf.to_excel('result/taxonIDmappingdict_fOrder.xlsx', index=False)
fdata.to_csv('Datasets/processing_4_micrOrder/micro_combineOrder_790row_191col.csv', index=False)
fdata_trans.to_csv('Datasets/processing_4_micrOrder/microOrder_trans.csv', index=False)
